...
GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_Roles_RoleID_Status] ON [dbo].[Roles] ([RoleID], [Status]) INCLUDE ([AutoAssignment], [BillingFrequency], [BillingPeriod], [Description], [IconFile], [IsPublic], [PortalID], [RoleGroupID], [RoleName], [RSVPCode], [SecurityMode], [ServiceFee], [TrialFee], [TrialFrequency], [TrialPeriod]) ON [PRIMARY]
GO
ALTER TABLE [dbo].[Roles] WITH NOCHECK ADD CONSTRAINT [FK_Roles_Portals] FOREIGN KEY ([PortalID]) REFERENCES [dbo].[Portals] ([PortalID]) ON DELETE CASCADE
GO
ALTER TABLE [dbo].[Roles] WITH NOCHECK ADD CONSTRAINT [FK_Roles_RoleGroups] FOREIGN KEY ([RoleGroupID]) REFERENCES [dbo].[RoleGroups] ([RoleGroupID])
GO
